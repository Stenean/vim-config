...
if __name__ == '__main__':
    try:
        ...
        sys.stderr.write("\nTraceback 1(most recent call first):\n")
        tb = traceback.format_tb(exceptionTraceback)
        for line in reversed(tb):
            sys.stderr.write(line)
        error = ''
        for line in reversed(tb):
            match = re.search('File \"(?P<file_name>.*)\", line (?P<line>\d+), in (?P<func_name>.*)', line)
            if match is not None:
                matches = match.groupdict()
                sys.stderr.write(u'%s|%s|%s' % (matches.get('file_name', ''), matches.get('line', ''), "".join(ex)))
